    ...
    <div class="fbneirong">
      <form class="am-form" action="/index.php/Admin/Role/edt/id/4" method="post">
        <div class="am-form-group am-cf">
          ...
          <div class="you">
            <input type="text" name="role_name" value="<?php echo ($role_info["role_name"]); ?>" class="am-input-sm" id="doc-ipt-email-1" placeholder="请输入角色名称">
          ...
          <table class="am-table am-table-bordered am-table-striped">
             ...
             <tbody>
               <?php if(is_array($authData)): $i = 0; $__LIST__ = $authData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i; if($vo1["auth_pid"] == 0): ?><tr>
                     <td style="text-align: center;"><label class="am-checkbox-inline"><input type="checkbox" name="role_auth_ids[]" value="<?php echo ($vo1["auth_id"]); ?>" <?php echo strpos(','. $role_info['role_auth_ids'] . ',' ,',' . $vo1['auth_id'] . ',')!==false?'checked':''; ?> ><?php echo ($vo1["auth_name"]); ?></label></td>
                     <td>
                    <?php if(is_array($authData)): $i = 0; $__LIST__ = $authData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i; if($vo2["auth_pid"] == $vo1["auth_id"] ): ?><label class="am-checkbox-inline"><input type="checkbox" name="role_auth_ids[]" value="<?php echo ($vo2["auth_id"]); ?>" <?php echo strpos(','. $role_info['role_auth_ids'] . ',' ,',' . $vo2['auth_id'] . ',')!==false?'checked':''; ?> ><?php echo ($vo2["auth_name"]); ?></label><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </td>
