    ...
    <div class="fbneirong">
      <form class="am-form" action="/index.php/Admin/Category/edt/id/23" method="post">
        <div class="am-form-group am-cf">
          ...
          <div class="you">
            <input type="text" name="cate_name" value="<?php echo ($cateInfo["cate_name"]); ?>" class="am-input-sm" id="doc-ipt-email-1" placeholder="请输入栏目名称">
            <input type="hidden" name="cate_id" value="<?php echo ($cateInfo["cate_id"]); ?>">
          </div>
        </div>
        <div class="am-form-group am-cf">
          ...
          <div class="you">
            <select name="cate_pid">
              ...
              <?php if(is_array($cateData)): $i = 0; $__LIST__ = $cateData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!in_array($vo['cate_id'], $cateChildList)): ?><option value="<?php echo ($vo["cate_id"]); ?>" <?php echo $cateInfo['cate_pid']==$vo['cate_id']?'selected':'' ?> ><?php echo (str_repeat('&emsp;',$vo["level"]*2)); echo ($vo["cate_name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
        </div>
        <div class="am-form-group am-cf">
